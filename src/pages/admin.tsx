        ...
        return (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div
                className={`glass-card p-6 rounded-xl border ${outOfStockItems.length > 0 ? "border-red-500/50" : "border-border/50"}`}
              >
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <p className="text-xs text-muted-foreground uppercase tracking-widest mb-1">
                  ...
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center ${outOfStockItems.length > 0 ? "bg-red-500/10 text-red-500" : "bg-secondary text-foreground"}`}
                  >
              ...
              <div className="glass-card p-6 rounded-xl border border-border/50">
                ...
                <div className="space-y-4">
                  {orders.slice(0, 4).map((order) => (
                    <div
                      key={order.id}
                      className="flex justify-between items-center p-3 rounded bg-secondary/30 hover:bg-secondary/50 transition-colors"
                    >
                      <div>
                      ...
                      <div className="text-right">
                        ...
                        <span
                          className={`text-[10px] uppercase px-2 py-1 rounded-full ${order.status === "Delivered" ? "bg-green-500/20 text-green-500" : order.status === "Processing" ? "bg-yellow-500/20 text-yellow-500" : "bg-secondary text-muted-foreground"}`}
                        >
                          {order.status}
                        </span>
                      </div>
                ...
                <div className="space-y-4">
                  {lowStockItems.length > 0 ? (
                    lowStockItems.map((item) => (
                      <div
                        key={item.id}
                        className="flex gap-4 items-center p-3 rounded bg-secondary/30 hover:bg-secondary/50 transition-colors"
                      >
                ...
                <tbody className="divide-y divide-border/50">
                  {inventoryList.map((product) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        {product.stock === 0 ? (
                          <span className="text-[10px] uppercase px-2 py-1 rounded-full bg-red-500/20 text-red-500">
                            Out of Stock
                          </span>
                        ) : product.stock <= 3 ? (
                          <span className="text-[10px] uppercase px-2 py-1 rounded-full bg-yellow-500/20 text-yellow-500">
                            Low Stock
                          </span>
                        ) : (
                          <span className="text-[10px] uppercase px-2 py-1 rounded-full bg-green-500/20 text-green-500">
                            In Stock
                          </span>
                        )}
                      </td>
  ...
  return (
    <div className="min-h-screen bg-background pt-24 pb-20 flex">
      ...
      <aside className="w-64 border-r border-border/30 px-4 py-8 flex flex-col shrink-0 min-h-[calc(100vh-6rem)]">
        <h1 className="brand-font text-2xl text-primary mb-8 px-4">
          Admin Panel
        </h1>
