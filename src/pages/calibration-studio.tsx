...
import { Save, AlertTriangle, CheckCircle, Image as ImageIcon, Play, Pause } from 'lucide-react';
...
const BLANK_PRODUCT: JewelryMetadata = {
  ...
  trackingRequirements: JSON.parse(JSON.stringify(DEFAULT_TRACKING_REQUIREMENTS.necklace)),
...
export default function CalibrationStudioPage() {
  ...
  const generatePreview = async () => {
    ...
    if (valid) {
      const preview = await PreviewGenerator.generateMannequinPreview(metadata, 300, 400);
  ...
  const handleCalibrationChange = (field: keyof typeof DEFAULT_CALIBRATION, value: number) => {
    setMetadata(prev => ({
      ...prev,
  ...
  const handleAnchorChange = (field: 'pivot' | 'offsetUnits', sub: 'x' | 'y', value: number) => {
    setMetadata(prev => ({
      ...prev,
      anchors: {
        ...prev.anchors,
    ...
    <div className="min-h-screen bg-gray-900 text-gray-100 flex flex-col font-sans">
      <header className="bg-gray-800 p-4 border-b border-gray-700 flex justify-between items-center">
        <h1 className="text-xl font-bold flex items-center gap-2 text-primary">
           Calibration Studio <span className="text-xs bg-primary/20 px-2 py-0.5 rounded">v1.5</span>
        </h1>
        <div className="flex gap-2">
          <button onClick={validateProduct} className="btn-secondary px-3 py-1.5 rounded text-sm flex items-center gap-1">
            <CheckCircle size={14} /> Validate
          </button>
          <button onClick={generatePreview} className="btn-secondary px-3 py-1.5 rounded text-sm flex items-center gap-1">
            <ImageIcon size={14} /> Preview
          </button>
          <button onClick={exportJson} className="bg-green-600 hover:bg-green-500 text-white px-4 py-1.5 rounded text-sm font-bold flex items-center gap-1 transition">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-80 bg-gray-800 border-r border-gray-700 flex flex-col p-4 overflow-y-auto">
          <h2 className="text-sm uppercase tracking-wider text-gray-400 mb-4 font-bold">1. Asset</h2>
          
          <div className="mb-6">
             <label className="block text-xs mb-1">Base Image (PNG)</label>
             <input type="file" accept="image/png" onChange={handleImageUpload} className="text-sm w-full bg-gray-900 p-2 rounded border border-gray-700" />
          </div>

          <h2 className="text-sm uppercase tracking-wider text-gray-400 mb-4 font-bold">2. Calibration (Fine-tune)</h2>
          
          <div className="space-y-4 mb-6">
            <div>
              <label className="flex justify-between text-xs mb-1">
                <span>Scale Multiplier</span>
                <span className="text-primary">{metadata.calibration.scaleCorrection.toFixed(2)}x</span>
              </label>
              <input type="range" min="0.1" max="3" step="0.05" 
                value={metadata.calibration.scaleCorrection} 
                onChange={e => handleCalibrationChange('scaleCorrection', parseFloat(e.target.value))}
            ...
            <div>
              <label className="flex justify-between text-xs mb-1">
                ...
                <span className="text-primary">{metadata.calibration.rotationCorrectionRad.toFixed(2)}</span>
              </label>
              <input type="range" min="-3.14" max="3.14" step="0.01" 
                value={metadata.calibration.rotationCorrectionRad} 
                onChange={e => handleCalibrationChange('rotationCorrectionRad', parseFloat(e.target.value))}
                className="w-full accent-primary" />
            </div>
          </div>

          <h2 className="text-sm uppercase tracking-wider text-gray-400 mb-4 font-bold">3. Anchor Def</h2>
          
          <div className="space-y-4 mb-6">
            <div>
              <label className="flex justify-between text-xs mb-1">
                <span>Pivot X (0-1)</span>
                <span className="text-primary">{metadata.anchors.pivot.x.toFixed(2)}</span>
              </label>
              <input type="range" min="0" max="1" step="0.05" 
                value={metadata.anchors.pivot.x} 
                onChange={e => handleAnchorChange('pivot', 'x', parseFloat(e.target.value))}
                className="w-full accent-primary" />
            </div>
            <div>
              <label className="flex justify-between text-xs mb-1">
                <span>Pivot Y (0-1)</span>
                <span className="text-primary">{metadata.anchors.pivot.y.toFixed(2)}</span>
              </label>
              <input type="range" min="0" max="1" step="0.05" 
                value={metadata.anchors.pivot.y} 
                onChange={e => handleAnchorChange('pivot', 'y', parseFloat(e.target.value))}
                className="w-full accent-primary" />
            </div>
            ...
            <div className="pt-2 border-t border-gray-700">
              <label className="flex justify-between text-xs mb-1">
                ...
                <span className="text-primary">{metadata.anchors.offsetUnits.y.toFixed(2)}</span>
              </label>
              <input type="range" min="-1" max="2" step="0.05" 
                value={metadata.anchors.offsetUnits.y} 
                onChange={e => handleAnchorChange('offsetUnits', 'y', parseFloat(e.target.value))}
                className="w-full accent-primary" />
            </div>
          </div>
        ...
        <div className="flex-1 bg-black flex flex-col p-4 relative">
          ...
          <div className="absolute top-6 right-6 z-10 flex gap-2">
            <button 
               onClick={() => { replayManager.setLoop(!isPlaying); setIsPlaying(!isPlaying); }}
        ...
        <div className="w-64 bg-gray-800 border-l border-gray-700 p-4 flex flex-col">
          <h2 className="text-sm uppercase tracking-wider text-gray-400 mb-4 font-bold">Generated Assets</h2>
          
          {previewUrl ? (
            <div className="bg-gray-900 rounded border border-gray-700 p-2 text-center">
              <img src={previewUrl} alt="Preview" className="mx-auto rounded w-[150px] bg-[#f0f0f0]" />
              <p className="text-[10px] text-gray-500 mt-2">preview.webp (300x400)</p>
            </div>
          ) : (
