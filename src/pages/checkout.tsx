...
export default function CheckoutPage() {
  ...
  const { cart, cartTotal, clearCart, couponApplied, couponDiscount, couponCode, placeOrder } = useAppContext();
  ...
  if (isSuccess) {
    return (
      <div className="min-h-screen bg-background pt-32 pb-20 flex flex-col items-center justify-center text-center px-4">
        ...
        >
          We have sent a confirmation email with your order details. Your exquisite pieces are being prepared by our artisans.
        </motion.p>
  ...
  return (
    <div className="min-h-screen bg-background pt-24 pb-20">
      <div className="container mx-auto px-4 max-w-6xl">
        <h1 className="brand-font text-4xl text-foreground mb-10 text-center md:text-left border-b border-border/30 pb-6">Secure Checkout</h1>
        ...
        <div className="flex flex-col lg:flex-row gap-12">
            ...
            <form onSubmit={handleSubmit} id="checkout-form" className="space-y-10">
              ...
              <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="space-y-1 md:col-span-2">
                    ...
                    <input required type="email" className="w-full bg-card border border-border rounded-sm py-3 px-4 text-sm focus:outline-none focus:border-primary transition-colors" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs uppercase tracking-widest text-muted-foreground">First Name *</label>
                    <input required name="firstName" type="text" className="w-full bg-card border border-border rounded-sm py-3 px-4 text-sm focus:outline-none focus:border-primary transition-colors" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs uppercase tracking-widest text-muted-foreground">Last Name *</label>
                    <input required name="lastName" type="text" className="w-full bg-card border border-border rounded-sm py-3 px-4 text-sm focus:outline-none focus:border-primary transition-colors" />
                  </div>
                  <div className="space-y-1 md:col-span-2">
                    <label className="text-xs uppercase tracking-widest text-muted-foreground">Street Address *</label>
                    <input required type="text" className="w-full bg-card border border-border rounded-sm py-3 px-4 text-sm focus:outline-none focus:border-primary transition-colors" />
                  </div>
                  <div className="space-y-1">
                  ...
                  <div className="space-y-1">
                    ...
                    <select required className="w-full bg-card border border-border rounded-sm py-3 px-4 text-sm focus:outline-none focus:border-primary transition-colors appearance-none">
                  ...
                  <div className="space-y-1">
                    ...
                    <input required type="tel" className="w-full bg-card border border-border rounded-sm py-3 px-4 text-sm focus:outline-none focus:border-primary transition-colors" />
                  </div>
                </div>
              </section>
              ...
              <section>
                ...
                <div className="flex gap-4 mb-6">
                  <label className={`flex-1 flex items-center justify-center gap-2 border rounded-lg p-4 cursor-pointer transition-colors ${paymentMethod === 'card' ? 'border-primary bg-primary/10' : 'border-border/50 bg-card hover:border-primary/50'}`}>
                    ...
                    <span className="text-sm uppercase tracking-widest font-medium">Card Payment</span>
                  </label>
                  <label className={`flex-1 flex items-center justify-center gap-2 border rounded-lg p-4 cursor-pointer transition-colors ${paymentMethod === 'cod' ? 'border-primary bg-primary/10' : 'border-border/50 bg-card hover:border-primary/50'}`}>
                    <input type="radio" name="paymentMethod" value="cod" checked={paymentMethod === 'cod'} onChange={() => setPaymentMethod('cod')} className="sr-only" />
                    <span className="text-sm uppercase tracking-widest font-medium">Cash on Delivery</span>
                  </label>
                </div>
                  ...
                  {paymentMethod === 'card' ? (
                    <>
                      <div className="flex justify-between items-center mb-6">
                        <span className="text-sm font-medium tracking-wider uppercase text-foreground">Credit / Debit Card</span>
                        <div className="flex gap-2 opacity-50 grayscale mix-blend-screen">
                          <div className="h-5 w-8 bg-white rounded flex items-center justify-center text-[6px] text-black font-bold">VISA</div>
                          <div className="h-5 w-8 bg-white rounded flex items-center justify-center text-[6px] text-black font-bold">MC</div>
                        </div>
                      </div>
                      ...
                      <div className="space-y-5 relative z-10">
                        <div className="space-y-1">
                          ...
                          <div className="relative">
                            <input required type="text" placeholder="0000 0000 0000 0000" maxLength={19} className="w-full bg-secondary border border-border rounded-sm py-3 px-4 text-sm focus:outline-none focus:border-primary transition-colors font-mono tracking-widest" />
                          </div>
                        </div>
                        <div className="space-y-1">
                          <label className="text-xs uppercase tracking-widest text-muted-foreground">Name on Card *</label>
                          <input required type="text" className="w-full bg-secondary border border-border rounded-sm py-3 px-4 text-sm focus:outline-none focus:border-primary transition-colors uppercase" />
                        </div>
                        <div className="grid grid-cols-2 gap-5">
                          <div className="space-y-1">
                            <label className="text-xs uppercase tracking-widest text-muted-foreground">Expiry (MM/YY) *</label>
                            <input required type="text" placeholder="MM/YY" maxLength={5} className="w-full bg-secondary border border-border rounded-sm py-3 px-4 text-sm focus:outline-none focus:border-primary transition-colors text-center" />
                          </div>
                          <div className="space-y-1">
                            <label className="text-xs uppercase tracking-widest text-muted-foreground">CVV *</label>
                            <input required type="password" placeholder="***" maxLength={4} className="w-full bg-secondary border border-border rounded-sm py-3 px-4 text-sm focus:outline-none focus:border-primary transition-colors text-center tracking-widest" />
                          </div>
                        </div>
                      </div>
                      
                      <div className="mt-6 flex items-center justify-center gap-2 text-xs text-muted-foreground uppercase tracking-widest">
                        <ShieldCheck size={14} className="text-green-500" /> Secure 256-bit Encryption
                      </div>
                    </>
                  ) : (
                    <div className="flex flex-col items-center justify-center py-8 text-center relative z-10">
                      ...
                      <h3 className="text-lg font-medium text-foreground mb-2">Pay on Delivery</h3>
                      <p className="text-sm text-muted-foreground max-w-sm">
                        You can pay in cash or via UPI to our delivery executive when your exquisite pieces arrive at your doorstep.
                      </p>
                    </div>
                  )}
                </div>
          ...
          <div className="w-full lg:w-2/5">
            <div className="glass-card p-6 md:p-8 rounded-xl border border-primary/20 sticky top-28">
              <h3 className="brand-font text-xl text-foreground mb-6 pb-4 border-b border-border/50">Your Order</h3>
                  ...
                  return (
                    <div key={p.id} className="flex gap-4 items-center">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="text-sm text-foreground truncate">{p.name}</h4>
                        <p className="text-xs text-muted-foreground uppercase">Qty: {item.quantity}</p>
                      </div>
              ...
              <div className="space-y-3 pt-6 border-t border-border/50 text-sm mb-6">
                ...
                {couponApplied && (
                  <div className="flex justify-between text-primary">
                    <span className="uppercase tracking-widest text-xs">Discount ({couponCode})</span>
              ...
              <div className="flex justify-between items-end pt-4 border-t border-border/50 mb-8">
                <span className="text-foreground uppercase tracking-widest text-sm font-semibold">Total to Pay</span>
