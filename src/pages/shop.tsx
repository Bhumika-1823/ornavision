...
import { Search, SlidersHorizontal, Star, Heart, Sparkles, ShoppingBag } from 'lucide-react';
...
export default function ShopPage() {
  ...
  const MAX_POSSIBLE_PRICE = useMemo(() => Math.max(500, ...PRODUCTS.map(p => p.price)), []);
  ...
  const filteredProducts = useMemo(() => {
    ...
    result = [...result].sort((a, b) => {
      switch (sortBy) {
        ...
        case 'rating': return b.ratingsAvg - a.ratingsAvg;
        case 'newest':
        default: return 0; // Keeping original order as "newest" for mock data
      }
    });
      ...
      <div className="bg-secondary/30 border-b border-border/30 py-12 mb-8">
        <div className="container mx-auto px-4 text-center">
          <h1 className="brand-font text-4xl md:text-5xl text-foreground mb-4">
            {category === 'all' ? 'All Collections' : CATEGORIES.find(c => c.slug === category)?.name}
          </h1>
        ...
        <aside className="w-full lg:w-64 shrink-0 space-y-8">
          ...
          <div>
            <h3 className="brand-font text-lg text-foreground mb-4 flex items-center gap-2 border-b border-border/30 pb-2">
              <SlidersHorizontal size={16} className="text-primary" /> Categories
            </h3>
            <div className="space-y-2">
              <label className="flex items-center gap-3 cursor-pointer group">
                <div className={`w-4 h-4 rounded-full border flex items-center justify-center transition-colors ${category === 'all' ? 'border-primary' : 'border-muted-foreground group-hover:border-primary/50'}`}>
                  {category === 'all' && <div className="w-2 h-2 rounded-full bg-primary" />}
                </div>
                <input type="radio" className="hidden" checked={category === 'all'} onChange={() => setCategory('all')} />
                <span className={`text-sm tracking-wider uppercase ${category === 'all' ? 'text-primary' : 'text-muted-foreground group-hover:text-foreground'}`}>All Pieces</span>
              </label>
              {CATEGORIES.map(cat => (
                <label key={cat.id} className="flex items-center gap-3 cursor-pointer group">
                  <div className={`w-4 h-4 rounded-full border flex items-center justify-center transition-colors ${category === cat.slug ? 'border-primary' : 'border-muted-foreground group-hover:border-primary/50'}`}>
                    {category === cat.slug && <div className="w-2 h-2 rounded-full bg-primary" />}
                  </div>
                  <input type="radio" className="hidden" checked={category === cat.slug} onChange={() => setCategory(cat.slug)} />
                  <span className={`text-sm tracking-wider uppercase ${category === cat.slug ? 'text-primary' : 'text-muted-foreground group-hover:text-foreground'}`}>{cat.name}</span>
                </label>
              ))}
            </div>
          </div>
          ...
          {/* Price Range */}
          <div>
            <h3 className="brand-font text-lg text-foreground mb-4 border-b border-border/30 pb-2">Maximum Price</h3>
          ...
          <div>
            <label className="flex items-start gap-3 cursor-pointer group p-3 bg-primary/5 border border-primary/20 rounded-md">
              <div className={`w-5 h-5 rounded flex-shrink-0 border flex items-center justify-center transition-colors mt-0.5 ${tryOnOnly ? 'bg-primary border-primary' : 'border-muted-foreground group-hover:border-primary/50'}`}>
                {tryOnOnly && <Sparkles size={12} className="text-black" />}
              </div>
              <input type="checkbox" className="hidden" checked={tryOnOnly} onChange={(e) => setTryOnOnly(e.target.checked)} />
              <div>
                <span className={`block text-sm font-semibold tracking-wider uppercase ${tryOnOnly ? 'text-primary' : 'text-foreground'}`}>Virtual Try-On</span>
                <span className="text-xs text-muted-foreground">Show only products with AR overlay available</span>
              </div>
            </label>
          </div>
        </aside>
        ...
        <main className="flex-1">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4 border-b border-border/30 pb-4">
            <span className="text-sm text-muted-foreground uppercase tracking-widest">
              Showing <span className="text-foreground font-bold">{filteredProducts.length}</span> pieces
            </span>
          ...
          {filteredProducts.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <h3 className="brand-font text-xl text-foreground mb-2">No pieces found</h3>
              <p className="text-muted-foreground max-w-sm">We couldn't find any pieces matching your current filters. Try adjusting them.</p>
              <button 
                onClick={() => { setCategory('all'); setSearch(''); setPriceRange(MAX_POSSIBLE_PRICE); setTryOnOnly(false); }}
                ...
                {filteredProducts.map((product) => (
                  ...
                  >
                    <div className="aspect-[4/5] bg-black relative overflow-hidden">
                      ...
                      <Link href={`/shop/${product.slug}`} className="absolute inset-0 flex items-center justify-center p-6 mix-blend-screen">
                        <img 
                    ...
                    <div className="p-5 flex flex-col flex-1">
                      <div className="flex justify-between items-start mb-2">
                        <Link href={`/shop/${product.slug}`} className="flex-1">
                          <h3 className="brand-font text-lg text-foreground group-hover:text-primary transition-colors leading-tight">{product.name}</h3>
                        </Link>
                      </div>
                      <p className="text-xs text-muted-foreground uppercase tracking-widest mb-4">{product.category}</p>
                      
                      <div className="mt-auto flex justify-between items-end">
                        <p className="text-primary font-medium tracking-wide text-lg">₹{product.price.toLocaleString()}</p>
                        <div className="flex items-center gap-1 text-xs text-muted-foreground">
                          <Star size={12} className="text-primary fill-primary" /> {product.ratingsAvg}
                          <span className="opacity-50">({product.ratingsCount})</span>
                        </div>
                      </div>
                    </div>
                  </motion.div>
