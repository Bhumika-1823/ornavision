...
export class CalibrationMetrics {
  ...
  static calculatePointDrift(frames: FrameState[], getPoint: (f: FrameState) => Point2D | null): DriftReport {
  ...
  static getEarringAnchorDrift(frames: FrameState[], side: 'left' | 'right'): DriftReport {
    return this.calculatePointDrift(frames, f => 
      f.face ? (side === 'left' ? f.face.leftEar : f.face.rightEar) : null
    );
  }

  static calculateValueStability(frames: FrameState[], getValue: (f: FrameState) => number | null): { maxDelta: number; averageDelta: number } {
