...
import { Point3D, Point2D, FingerState, HandState, FrameState } from '../../types';
...
describe('Ring Engine RC9', () => {
  ...
  it('computes Palm Normal correctly (Hand Rolled Away)', () => {
    ...
    // Test right hand
    const normal = HandEstimator.computePalmNormal(indexMcp, pinkyMcp, wrist, 'Right');
  ...
  it('fades ring opacity to 0 when finger is curled into a fist', () => {
    ...
    const mockHand: HandState = {
      ...
      fingers: {
        thumb: mockFinger, index: mockFinger, middle: mockFinger, pinky: mockFinger,
        ring: mockFinger
      },
      palmWidthPx: 80
    };
    ...
    const transform = computeRingTransform(mockMeta, mockFrame, mockAsset, { scale: 1, offsetX: 0, offsetY: 0 });
