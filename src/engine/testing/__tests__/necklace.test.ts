...
describe("Necklace Engine Phase 2", () => {
  ...
  it("validates subcategory anchor independence", async () => {
    const report =
      await NecklaceValidationSuite.validateSubcategories(createMockMeta);
  ...
  describe("NeckEstimator and Anatomy", () => {
    ...
    it("handles wide faces accurately", () => {
      ...
      const metrics = NeckEstimator.estimate(frame);
      expect(metrics!.neckWidthPx).toBeGreaterThan(
        FaceLibrary.getSmallFace().face!.faceWidthPx * 0.75,
      );
    });
  ...
  describe("Pendant Engine (RC3)", () => {
    ...
    it("inherits parent transform and maintains zero detachment error", () => {
      const frame = FaceLibrary.getAverageFace();
      const { transform, metrics } = PendantEngine.compute(
        mockParentTransform,
        mockPendantMeta,
        frame,
      );
    ...
    it("simulates damped harmonic swing when parent moves", () => {
      ...
      const { metrics } = PendantEngine.compute(
        movedParent,
        mockPendantMeta,
        frame,
      );
    ...
    it("applies gravity rotation based on body rotation", () => {
      const frame = FaceLibrary.getAverageFace();
      frame.neckMetrics = {
        ...NeckEstimator.estimate(frame)!,
        bodyRotation: -0.2,
      };

      const { transform } = PendantEngine.compute(
        mockParentTransform,
        mockPendantMeta,
        frame,
      );
