...
describe("Bracelet & Watch Engine RC10", () => {
  ...
  it("applies bangle vs rigid physics profiles", () => {
    ...
    const watchMeta = {
      ...mockMetaBase,
      category: "watch" as any,
      braceletSpec: { physicsProfile: "rigid" as const },
    };
    const watchTransform = computeBraceletTransform(
      watchMeta,
      mockFrame,
      mockAsset,
    ...
    const bangleMeta = {
      ...mockMetaBase,
      braceletSpec: { physicsProfile: "bangle" as const },
    };
    const bangleTransform = computeBraceletTransform(
      bangleMeta,
      mockFrame,
      mockAsset,
      { scale: 1, offsetX: 0, offsetY: 0 },
