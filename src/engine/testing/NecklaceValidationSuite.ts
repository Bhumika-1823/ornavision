...
export class NecklaceValidationSuite {
   ...
   */
  static async validateSubcategories(
    createMockMeta: (sub: string) => JewelryMetadata,
  ): Promise<{
    passed: boolean;
    errors: string[];
  ...
  private static async testChokerStability(
    ...
    engine.setItems([
      { metadata: meta, userAdjust: { scale: 1, offsetX: 0, offsetY: 0 } },
    ]);
    ...
    if (maxDrift > 2) {
      errors.push(
        `Choker drift exceeded bounds during body jitter: ${maxDrift}px`,
      );
    }
  }

  private static async testChainIndependence(
    meta: JewelryMetadata,
    errors: string[],
    metrics: Record<string, any>,
  ) {
    ...
    if (maxDrift > 2) {
      errors.push(
        `Long chain drift exceeded bounds during head roll: ${maxDrift}px`,
      );
    }
  }
}
