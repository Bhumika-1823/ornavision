...
export function validateMetadata(meta: JewelryMetadata): ValidationResult {
  ...
  if (!(sanitized.defaultScale > 0)) {
    warnings.push(
      `defaultScale invalid (${sanitized.defaultScale}), defaulting to 1`,
    );
    sanitized.defaultScale = 1;
  }
  if (
    !(sanitized.minScale > 0) ||
    sanitized.minScale > sanitized.defaultScale
  ) {
  ...
  if (sanitized.perspectiveCompression.maxCompression > 0.9) {
    warnings.push(
      "perspectiveCompression.maxCompression too aggressive, clamping to 0.9",
    );
