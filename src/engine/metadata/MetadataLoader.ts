 ...
 */
export function loadMetadataForProduct(
  product: LoadableProduct,
): JewelryMetadata | null {
  ...
  const signature = JSON.stringify(
    product.tryonMetadataRich ?? product.tryonMetadata ?? null,
  );
  ...
  if (errors.length > 0) {
    console.error(
      `[MetadataLoader] Invalid metadata for "${product.id}":`,
      errors,
    );
    return null;
  }
  if (warnings.length > 0) {
    console.warn(
      `[MetadataLoader] Metadata warnings for "${product.id}":`,
      warnings,
    );
  }
