...
export class TryonEngine {
  ...
  start(opts: EngineStartOptions): void {
    console.log("[Ornavision] start() called —", {
      mode: opts.mode,
      isReady: this.isReady(),
      hasCanvas: !!opts.canvasElement,
      hasVideo: !!opts.videoElement,
      hasImage: !!opts.imageElement,
    });
    if (!this.isReady() || !opts.canvasElement) {
      console.warn(
        "[Ornavision] start() ABORTED — isReady:",
        this.isReady(),
        "hasCanvas:",
        !!opts.canvasElement,
      );
      ...
      this.cameraManager
        .start(async () => {
          ...
          if (occlusionEngine.enabled)
            await occlusionEngine.send(opts.videoElement!);
        })
        .catch((err: any) => {
    ...
    if (opts.mode === "photo" && opts.imageElement) {
      ...
      const processPhoto = async () => {
        ...
        if (occlusionEngine.enabled)
          await occlusionEngine.send(opts.imageElement!);
      };
      this.photoIntervalId = window.setInterval(processPhoto, 300);
      processPhoto();
    }

    opts.canvasElement.addEventListener(
      "webglcontextlost",
  ...
  stop(): void {
    ...
    if (this.opts?.canvasElement) {
      this.opts.canvasElement.removeEventListener(
        "webglcontextlost",
        this.boundContextLost,
      );
      this.opts.canvasElement.removeEventListener(
  ...
  private renderFrame(): void {
    ...
    if (this._logCount % 120 === 0) {
      ...
      console.log("[Ornavision] renderFrame #" + this._logCount, {
        ...
        items: this.items.map(
          (i) => i.metadata.id + " (" + i.metadata.category + ")",
        ),
      });
    }
    ...
    ctx.fillStyle = "#0f0";
    ctx.fillText(
      `Face: ${frame.face ? "DETECTED ✓" : "NOT DETECTED ✗"}`,
      8,
      18,
    );
    ctx.fillText(
      `NeckMetrics: ${frame.neckMetrics ? "OK ✓" : "NONE ✗"}`,
      8,
      34,
    );
    ctx.fillText(
      `Items: ${this.items.length} [${this.items.map((i) => i.metadata.category).join(", ")}]`,
      8,
      50,
    );
    if (frame.face) {
      ctx.fillText(
        `Face pos: (${frame.face.center.x.toFixed(0)}, ${frame.face.center.y.toFixed(0)}) w=${frame.face.faceWidthPx.toFixed(0)}`,
        8,
        66,
      );
    }
...
function categoriesNeedFace(categories: Set<JewelryCategory>): boolean {
  return ["necklace", "earrings", "forehead", "nose_ring"].some((c) =>
    categories.has(c as JewelryCategory),
  );
}
