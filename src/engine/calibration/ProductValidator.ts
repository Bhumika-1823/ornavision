...
export class ProductValidator {
  ...
  static async validate(metadata: JewelryMetadata): Promise<ValidationReport> {
    ...
    try {
      ...
      if (!imgOk) errors.push(`Base image missing or unresolvable: ${metadata.image}`);
      
      if (metadata.mask) {
        const maskOk = await this.checkImageExists(metadata.mask);
        if (!maskOk) errors.push(`Mask image missing or unresolvable: ${metadata.mask}`);
      }
      
      if (metadata.shadow) {
        const shadowOk = await this.checkImageExists(metadata.shadow);
        if (!shadowOk) errors.push(`Shadow image missing or unresolvable: ${metadata.shadow}`);
      }
    } catch (e: any) {
    ...
    if (metadata.calibration.scaleCorrection <= 0) errors.push('scaleCorrection must be > 0');
