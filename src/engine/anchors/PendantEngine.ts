...
export class PendantEngine {
  static compute(
    ...
    const attachX =
      parentTransform.x + spec.attachmentPoint.x * parentTransform.width;
    const attachY =
      parentTransform.y + spec.attachmentPoint.y * parentTransform.height;
    ...
    state.angle = Math.max(
      -spec.maxSwing,
      Math.min(spec.maxSwing, state.angle),
    );
    ...
    state.filter.updateParams({
      minCutoff: quality < 0.5 ? 0.1 : 1.0,
      beta: 0.05,
    });
    ...
    const finalLocalRotation =
      smoothedAngle + gravityPull + spec.rotationOffset;

    // 4. Calculate Final Transform
    // Apply chain and center offsets, rotated by the final local rotation.
    const totalOffsetX =
      (spec.chainOffset.x + spec.centerOffset.x) * parentTransform.width;
    const totalOffsetY =
      (spec.chainOffset.y + spec.centerOffset.y) * parentTransform.height;
