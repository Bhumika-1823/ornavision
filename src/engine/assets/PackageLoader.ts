...
import { JewelryMetadata, DEFAULT_ANCHORS, DEFAULT_PERSPECTIVE, DEFAULT_LIGHTING, DEFAULT_CALIBRATION, DEFAULT_TRACKING_REQUIREMENTS, JewelryCategory, JewelrySubcategory } from '../metadata/JewelryMetadata';
...
export class PackageLoader {
   ...
   */
  static async load(pkg: ProductPackage, baseUrl = ''): Promise<JewelryMetadata> {
    ...
    const metadata: JewelryMetadata = {
      ...
      shadow: manifest.assets.shadow ? baseUrl + manifest.assets.shadow : undefined,
      thumbnail: manifest.assets.thumbnail ? baseUrl + manifest.assets.thumbnail : undefined,
      ...
      perspectiveCompression: cal.perspectiveCompression ?? { ...DEFAULT_PERSPECTIVE },
      ...
      trackingRequirements: cal.trackingRequirements ?? { ...DEFAULT_TRACKING_REQUIREMENTS[category] },
