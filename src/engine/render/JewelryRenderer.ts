...
export class JewelryRenderer {
  ...
  renderAll(items: RenderableItem[], frame: FrameState, mirror: boolean): void {
    const drawJobs: {
      transform: ReturnType<typeof anchorEngine.compute>[number];
      item: RenderableItem;
    }[] = [];
    ...
    for (const item of items) {
      ...
      const transforms = anchorEngine.compute(
        item.metadata,
        frame,
        asset,
        item.userAdjust,
    ...
    for (const job of drawJobs) {
      ...
      if (!asset) continue;
      this.canvasRenderer.drawSprite(
        asset,
        job.item.metadata,
        job.transform,
        frame.lighting,
      ...
      if (occlusionEngine.enabled) {
        ...
        if (job.item.metadata.category === "earrings") {
          ...
          job.item.metadata.category === "bracelet" ||
          job.item.metadata.category === "watch"
        ) {
