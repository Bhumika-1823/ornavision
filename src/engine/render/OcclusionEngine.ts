...
export type OcclusionRegion =
  "hair" | "face" | "neck" | "hands" | "body" | "clothes";
...
export class OcclusionEngine {
  ...
  public updateMasks(frame: FrameState): void {
    ...
    if (!bodyCtx || !faceCtx || !neckCtx || !hairCtx || !handCtx || !clothesCtx)
      return;
    ...
    bodyCtx.drawImage(
      this.latestSegmentation as any,
      0,
      0,
      this.width,
      this.height,
    );
    clothesCtx.drawImage(
      this.latestSegmentation as any,
      0,
      0,
      this.width,
    ...
    if (frame.neckMetrics) {
      ...
      if (frame.body) {
        pts.push(
          frame.body.rightShoulder,
          frame.body.chestCenter,
          frame.body.leftShoulder,
        );
      } else {
    ...
    if (frame.hands && frame.hands.length > 0) {
      ...
      for (const hand of frame.hands) {
        ...
        for (const fingerName of Object.keys(hand.fingers) as Array<
