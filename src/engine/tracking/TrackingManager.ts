...
import {
  LandmarkSmoother,
  ScalarSmootherBundle,
} from "../smoothing/LandmarkSmoother";
import {
  estimateHeadPose,
...
export class TrackingManager {
  ...
  private faceLandmarkSmoother = new LandmarkSmoother({
    minCutoff: 1.4,
    beta: 0.025,
    dCutoff: 1.0,
  });
  ...
  private poseLandmarkSmoother = new LandmarkSmoother({
    minCutoff: 1.0,
    beta: 0.015,
    dCutoff: 1.0,
  });
  ...
  async process(
    ...
    if (needs.face && this.faceTracker.isReady())
      jobs.push(this.faceTracker.send(image));
    if (needs.hands && this.handTracker.isReady())
      jobs.push(this.handTracker.send(image));
    if (needs.body && this.poseTracker.isReady())
      jobs.push(this.poseTracker.send(image));
  ...
  buildFrameState(width: number, height: number): FrameState {
    ...
    const body = this.buildBodyState(
      width,
      height,
      t,
      face?.faceWidthPx ?? null,
    );
  ...
  private buildFaceState(
    ...
    const rawLandmarks: Point3D[] | undefined =
    ...
    const eyeDistancePxVal = this.faceScalars.distance.update(
      calcEyeDistancePx(smoothed, width, height),
    );
  ...
  private buildHandState(
    ...
    rawHandsList.forEach((rawLandmarks, i) => {
      const label: "Left" | "Right" | "Unknown" =
      ...
      const palmNormal = HandEstimator.computePalmNormal(
        iMcp3D,
        pMcp3D,
        wrist3D,
        label,
  ...
  private buildBodyState(
    ...
    const leftElbow = smoothed[13]
      ? toPx(smoothed[13], width, height)
      : undefined;
    const rightElbow = smoothed[14]
      ? toPx(smoothed[14], width, height)
      : undefined;
    ...
    void bodyScaleReference(
      faceWidthHint ?? shoulderWidthPx / 2.8,
      shoulderWidthPx,
    );
